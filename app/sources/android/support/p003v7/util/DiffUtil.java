...
public class DiffUtil {
    private static final Comparator<Snake> SNAKE_COMPARATOR = new Comparator<Snake>() {
        public int compare(Snake o1, Snake o2) {
            int cmpX = o1.f35x - o2.f35x;
            return cmpX == 0 ? o1.f36y - o2.f36y : cmpX;
        }
    };
    ...
    public static DiffResult calculateDiff(Callback cb, boolean detectMoves) {
        ...
        while (!arrayList.isEmpty()) {
            ...
            if (snake != null) {
                ...
                snake.f35x += range.oldListStart;
                snake.f36y += range.newListStart;
                ...
                if (snake.reverse) {
                    left.oldListEnd = snake.f35x;
                    left.newListEnd = snake.f36y;
                } else if (snake.removal) {
                    left.oldListEnd = snake.f35x - 1;
                    left.newListEnd = snake.f36y;
                } else {
                    left.oldListEnd = snake.f35x;
                    left.newListEnd = snake.f36y - 1;
                }
                arrayList.add(left);
                Range right = range;
                if (!snake.reverse) {
                    right.oldListStart = snake.f35x + snake.size;
                    right.newListStart = snake.f36y + snake.size;
                } else if (snake.removal) {
                    right.oldListStart = snake.f35x + snake.size + 1;
                    right.newListStart = snake.f36y + snake.size;
                } else {
                    right.oldListStart = snake.f35x + snake.size;
                    right.newListStart = snake.f36y + snake.size + 1;
                }
                arrayList.add(right);
            } else {
        ...
        for (int d = 0; d <= dLimit; d++) {
            ...
            while (k <= d) {
                ...
                if (!checkInFwd || k < (delta - d) + 1 || k > (delta + d) - 1 || forward[kOffset + k] < backward[kOffset + k]) {
                    ...
                    outSnake.f35x = backward[kOffset + k];
                    outSnake.f36y = outSnake.f35x - k;
            ...
            while (k2 <= d) {
                ...
                if (checkInFwd || k2 + delta < (-d) || k2 + delta > d || forward[kOffset + backwardK] < backward[kOffset + backwardK]) {
                    ...
                    outSnake2.f35x = backward[kOffset + backwardK];
                    outSnake2.f36y = outSnake2.f35x - backwardK;
    ...
    public static class DiffResult {
        ...
        private void addRootSnake() {
            ...
            if (firstSnake == null || firstSnake.f35x != 0 || firstSnake.f36y != 0) {
        ...
        private void findMatchingItems() {
            ...
            for (int i = this.mSnakes.size() - 1; i >= 0; i--) {
                ...
                int endX = snake.f35x + snake.size;
                int endY = snake.f36y + snake.size;
                ...
                for (int j = 0; j < snake.size; j++) {
                    int oldItemPos = snake.f35x + j;
                    int newItemPos = snake.f36y + j;
        ...
        public void dispatchUpdatesTo(ListUpdateCallback updateCallback) {
            ...
            for (int snakeIndex = this.mSnakes.size() - 1; snakeIndex >= 0; snakeIndex--) {
                ...
                int endX = snake.f35x + snakeSize;
                int endY = snake.f36y + snakeSize;
                if (endX < posOld) {
                ...
                for (int i = snakeSize - 1; i >= 0; i--) {
                    if ((this.mOldItemStatuses[snake.f35x + i] & 31) == 2) {
                        batchingCallback.onChanged(snake.f35x + i, 1, this.mCallback.getChangePayload(snake.f35x + i, snake.f36y + i));
                    }
                }
                posOld = snake.f35x;
                posNew = snake.f36y;
